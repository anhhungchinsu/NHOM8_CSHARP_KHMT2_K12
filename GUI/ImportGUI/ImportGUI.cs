...
namespace GUI
{
    public partial class ImportGUI : Form
    {
        ...
        public ImportGUI(SimpleTcpClient client)
        {
            InitializeComponent();
            this.client = client;
        }
        ...
        private void btnLuuPN_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    if (client != null)
                    {
                        String log = txtTenNV.Text + " vừa nhập đơn " + txtSoPN.Text + " lúc " + I.Ngaylappn;
                        client.WriteLineAndGetReply(log, TimeSpan.FromSeconds(1));
                    }
                }
                catch
                {
